...
| REQ-AUDIT-001 | AuditEntry written on 6 of 7 actions | IMPLEMENTED (partial) | `src/modules/audit/service.ts` (`logAudit`), called from `src/plugins/auth.ts`, `src/modules/grants/service.ts`, `src/modules/visits/service.ts`, `src/modules/maternal/service.ts` | `test/patients.test.ts`, `test/grants.test.ts`, `test/visits.test.ts`, `test/maternal.test.ts` | `record_viewed` (Session 4), `grant_created`/`grant_redeemed`/`grant_revoked` (Session 5), `visit_added` (Session 7), `contact_recorded` (Session 9). Only `document_added` remains unwired, pending Documents (Phase 9) |
...
| REQ-SEED-001 | Seed facilities/invites/users/patients/visits/pregnancy/sms | NOT_STARTED | `prisma/seed.ts` | manual: `npm run seed` then inspect | codelists (REQ-SEED-003) and nothing else is seeded so far - the full demo dataset (facilities/invites/users/patients/visits/pregnancy) is still a content-authoring task |
...
| REQ-DOC-007 | AI worker | NOT_STARTED | `src/modules/documents/ai.worker.ts` | `test/documents.test.ts` | Tier 2; `summarizeDocument` returns 501 even with `AI_MODE=on` since this worker doesn't exist - queuing a job nothing will ever process would be a fake implementation (CLAUDE.md §2) |
...
| — | Magic-byte upload check follow-up | NOT_STARTED | `src/modules/documents/service.ts` | `test/documents.test.ts` | `docs/SECURITY.md` row 16 — documented gap; the Documents module now exists (Phase 9) but content is never inspected server-side (client declares `contentType`/`sizeBytes`, never verified against the actual uploaded bytes since the server never downloads them) |
