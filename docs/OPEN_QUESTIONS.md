...
**Implementation correction (Session 3):** the IP+phone control is a custom Redis fixed-window counter (`src/lib/rateLimiter.ts`), not `@fastify/rate-limit` as this line originally planned — that plugin's key generator runs at Fastify's `onRequest` hook, before the request body is parsed, so it cannot key on a phone number from the body at all. `@fastify/rate-limit` remains registered and in use for the global 300/min/IP default (`src/plugins/ratelimit.ts`, Session 2), which only needs the IP. Both controls' actual thresholds/semantics are unchanged from this resolution.

---

QUESTION 7: How should sessions get a real, reachable Postgres + Redis for tests (CLAUDE.md §10 forbids mocking the database)?
AFFECTS:    every REQ with a `test/*.test.ts` row that touches the database — as of Session 3, all of REQ-AUTH-*/REQ-ROLE-*/REQ-USER-*, and every later phase's tests too
WHY:        Two consecutive sessions (2 and 3) had no Docker, no WSL, and no `psql` in the execution sandbox; the only reachable Postgres is a pre-existing, unrelated instance on `localhost:5432` with unknown credentials, deliberately left untouched (Session 2's own decision, restated in Session 3). Redis has never been reachable at all (port 6379 closed both sessions). This isn't a design ambiguity the docs can resolve — it's this execution environment's actual infrastructure, which only the user can provide or grant access to.
OPTIONS:    A. Install Docker Desktop (or enable WSL2 + Docker) on this machine, then `docker compose up -d` per `backend/docker-compose.yml` — the path the project was designed around from Session 0 onward.
            B. Provide credentials/access to the existing native Postgres instance on :5432 (a new role + database for this project only, not touching what's already there), plus install/run a local Redis for Windows.
            C. Point `TEST_DATABASE_URL`/`REDIS_URL` at a remote/cloud Postgres + Redis (e.g. a free-tier managed instance) reachable from this machine.
